...
const EstadoFlujosEfectivo = () => {
    ...
    const calcularPerdidaUtilidad = (movimientos) => {
        ...
        const perdidaUtilidad = ((utilidadBruta - totalGastosGenerales)*-1);
    ...
    const clasificarCuentas = (totalesPorCuenta) => {
        const actividadesOperacion = ['Clientes', 'Mercancias', 'IVA acreditable', 'IVA por acreditar', 
                                    'Renta pagada por anticipado', 'Papeleria', 'IVA trasladado', 
                                    'IVA por trasladar'];
        
        const actividadesInversion = ['Mobiliario y equipo', 'Terrenos', 'Edificios', 'Equipo de computo'];
        
        const actividadesFinanciamiento = ['Capital social', 'Acreedores', 'Documentos por pagar'];
        ...
        Object.entries(totalesPorCuenta).forEach(([cuentaId, totales]) => {
            ...
            if (actividadesOperacion.includes(nombreCuenta)) {
                clasificadas.operacion.push({ nombre: nombreCuenta, saldo });
            } else if (actividadesInversion.includes(nombreCuenta)) {
                clasificadas.inversion.push({ nombre: nombreCuenta, saldo });
            } else if (actividadesFinanciamiento.includes(nombreCuenta)) {
                clasificadas.financiamiento.push({ nombre: nombreCuenta, saldo });
    ...
    const totalOperacion = cuentasClasificadas.operacion.reduce((sum, cuenta) => sum + cuenta.saldo, 0) + perdidaUtilidad;
    const totalInversion = cuentasClasificadas.inversion.reduce((sum, cuenta) => sum + cuenta.saldo, 0);
    const totalDepreciaciones = cuentasClasificadas.depreciaciones.reduce((sum, cuenta) => sum + cuenta.saldo, 0);
    const totalFinanciamiento = cuentasClasificadas.financiamiento.reduce((sum, cuenta) => sum + cuenta.saldo, 0) + totalDepreciaciones;
                    ...
                    {movimientos.length > 0 && (
                        <div className="mt-4">
                            ...
                            <table className="table table-bordered">
                                <thead>
                                    <tr className='text-center'>
                                        <th colSpan="2">Actividades en operación:</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {cuentasClasificadas.operacion.map((cuenta, index) => (
                                        <tr key={`operacion-${index}`}>
                                            <td>{cuenta.nombre}</td>
                                    ...
                                    <tr className="fw-bold">
                                        <td>Flujo neto del efectivo de actividades en operación</td>
                                        <td className="text-end">${totalOperacion.toFixed(2)}</td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <table className="table table-bordered mt-4">
                                <thead>
                                    <tr className='text-center'>
                                        <th colSpan="2">Actividades de inversión:</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {cuentasClasificadas.inversion.map((cuenta, index) => (
                                        <tr key={`inversion-${index}`}>
                                            <td>{cuenta.nombre}</td>
                                            <td className="text-end">${cuenta.saldo.toFixed(2)}</td>
                                        </tr>
                                    ))}
                                    <tr className="fw-bold">
                                        <td>Flujos netos de efectivo de inversión</td>
                                        <td className="text-end">${totalInversion.toFixed(2)}</td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <table className="table table-bordered mt-4">
                                <thead>
                                    <tr className='text-center'>
                                        <th colSpan="2">Actividades de financiamiento:</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {cuentasClasificadas.financiamiento.map((cuenta, index) => (
                                        <tr key={`financiamiento-${index}`}>
                                            <td>{cuenta.nombre}</td>
                                            <td className="text-end">${cuenta.saldo.toFixed(2)}</td>
                                        </tr>
                                    ))}
                                    ...
                                    <tr className="fw-bold">
                                        <td>Flujos netos de efectivo de actividades en financiamiento</td>
                                        <td className="text-end">${totalFinanciamiento.toFixed(2)}</td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <table className="table table-bordered mt-4">
                                <thead>
                                    <tr className='text-center'>
                                        <th colSpan="4">Incremento neto de efectivo y equivalentes de efectivo:</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td colSpan="2">Efectivo al principio del periodo</td>
                                        <td></td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">Efectivo al final del periodo</td>
                                        <td></td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td className='text-end'>${bancos-100000.00.toFixed(2)}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td colSpan="2">Efectivo al principio del periodo</td>
                                    ...
                                    <tr>
                                        ...
                                        <td className='text-end'>${caja-50000.00.toFixed(2)}</td>
                                        <td></td>
                                    </tr>
                                    <tr className="fw-bold">
                                        <td colSpan="2">Efectivo al final del periodo</td>
                                        <td></td>
                                        <td className="text-end">${(totalInversion+totalFinanciamiento + totalOperacion).toFixed(2)}</td>
                                    </tr>
                                    <tr className="fw-bold">
                                        <td colSpan="2">Suma de saldos finales en caja y bancos</td>
                                        <td></td>
                                        <td className="text-end">${(bancos + caja).toFixed(2)}</td>
                                    </tr>
                                </tbody>
                            </table>
