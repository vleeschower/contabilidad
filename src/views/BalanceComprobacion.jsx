...
const BalanceComprobacion = () => {
    ...
    // Calcular saldos en debe y haber
    const calcularSaldos = (debe, haber) => {
        if (debe > haber) {
            return { saldoDebe: debe - haber, saldoHaber: 0 };
        } else {
            return { saldoDebe: 0, saldoHaber: haber - debe };
        }
    };
    ...
    const totalesGenerales = Object.values(totalesPorCuenta).reduce(
        (totales, cuenta) => {
            const { saldoDebe, saldoHaber } = calcularSaldos(cuenta.debe, cuenta.haber);
            return {
                totalDebe: totales.totalDebe + cuenta.debe,
                totalHaber: totales.totalHaber + cuenta.haber,
                totalSaldoDebe: totales.totalSaldoDebe + saldoDebe,
                totalSaldoHaber: totales.totalSaldoHaber + saldoHaber,
            };
        },
                        ...
                        <div className="mt-4">
                            ...
                            <table className="table table-bordered">
                                    ...
                                    {Object.entries(totalesPorCuenta).map(([cuentaId, totales]) => {
                                        ...
                                        return (
                                            <tr key={cuentaId}>
                                                ...
                                                <td>${saldoDebe.toFixed(2)}</td>
                                                <td>${saldoHaber.toFixed(2)}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                                <tfoot>
                                    <tr>
                                        <th>Totales</th>
                                        <th>${totalesGenerales.totalDebe.toFixed(2)}</th>
                                        <th>${totalesGenerales.totalHaber.toFixed(2)}</th>
                                        <th>${totalesGenerales.totalSaldoDebe.toFixed(2)}</th>
                                        <th>${totalesGenerales.totalSaldoHaber.toFixed(2)}</th>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
